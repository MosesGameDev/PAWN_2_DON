...
public class UIDialogueElement : MonoBehaviour
{
    ...
    {
        StartCoroutine(ShowWithDelayEnum(delay));
    }


    IEnumerator ShowWithDelayEnum(float delay)
    {
    ...
    public void Show()
    {
        ...
        if (useTimeout)
        {
            StartCoroutine(HideAfterTimeout(delay + timeout));
    ...
    IEnumerator HideAfterTimeout(float delay)
    {
        yield return new WaitForSeconds(delay);
        Hide();
    }
